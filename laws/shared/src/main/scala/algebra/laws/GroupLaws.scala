...
trait GroupLaws[A] extends Laws {
  ...
  def semigroup(implicit A: Semigroup[A], check: IsSerializable[Semigroup[A]]) =
    new GroupProperties(
      name = "semigroup",
      parents = Nil,
  ...
  def band(implicit A: Band[A], check: IsSerializable[Band[A]]) =
    new GroupProperties(
      name = "band",
  ...
  def commutativeSemigroup(implicit A: CommutativeSemigroup[A],
    check: IsSerializable[CommutativeSemigroup[A]]) = new GroupProperties(
    name = "commutative semigroup",
      parents = List(semigroup),
      Rules.commutative(A.combine)
  )

  def semilattice(implicit A: Semilattice[A], check: IsSerializable[Semilattice[A]]) =
    new GroupProperties(
      name = "semilattice",
      parents = List(band, commutativeSemigroup)
    )

  def monoid(implicit A: Monoid[A], check: IsSerializable[Monoid[A]]) =
    new GroupProperties(
      name = "monoid",
      parents = List(semigroup),
  ...
  def commutativeMonoid(implicit A: CommutativeMonoid[A]
  ...
  def boundedSemilattice(implicit A: BoundedSemilattice[A],
  ...
  def group(implicit A: Group[A], check: IsSerializable[Group[A]]) =
    new GroupProperties(
      name = "group",
      parents = List(monoid),
  ...
  def commutativeGroup(implicit A: CommutativeGroup[A],
  ...
  def additiveSemigroup(implicit A: AdditiveSemigroup[A],
    semiCheck: IsSerializable[Semigroup[A]],
    addCheck: IsSerializable[AdditiveSemigroup[A]]) =
    new AdditiveProperties(
      base = semigroup(A.additive, semiCheck),
      parents = Nil,
      Rules.serializable(A),
  ...
  def additiveCommutativeSemigroup(implicit A: AdditiveCommutativeSemigroup[A],
    ...
    new AdditiveProperties(
      base = commutativeSemigroup(A.additive, check),
      parents = List(additiveSemigroup)
    )

  def additiveMonoid(implicit A: AdditiveMonoid[A],
    check: IsSerializable[Monoid[A]],
    addCheck: IsSerializable[AdditiveMonoid[A]]) =
    new AdditiveProperties(
      base = monoid(A.additive, check),
      parents = List(additiveSemigroup),
      Rules.repeat0("sumN", "zero", A.zero)(A.sumN),
      Rules.collect0("sum", "zero", A.zero)(A.sum)
    )

  def additiveCommutativeMonoid(implicit A: AdditiveCommutativeMonoid[A],
    check: IsSerializable[CommutativeMonoid[A]],
    addCheck: IsSerializable[AdditiveCommutativeMonoid[A]]) =
    new AdditiveProperties(
      base = commutativeMonoid(A.additive, check),
      parents = List(additiveMonoid)
    )

  def additiveGroup(implicit A: AdditiveGroup[A],
    check: IsSerializable[Group[A]],
    addCheck: IsSerializable[AdditiveGroup[A]]) =
    new AdditiveProperties(
      base = group(A.additive, check),
      parents = List(additiveMonoid),
      Rules.consistentInverse("subtract")(A.minus)(A.plus)(A.negate)
    )

  def additiveCommutativeGroup(implicit A: AdditiveCommutativeGroup[A],
    check: IsSerializable[CommutativeGroup[A]],
    addCheck: IsSerializable[AdditiveCommutativeGroup[A]]) =
    new AdditiveProperties(
      base = commutativeGroup(A.additive, check),
